...
class _ClassBuilder(object):
    ...
    def __init__(self, cls, these, slots, frozen, auto_attribs):
        attrs, super_attrs, super_map = _transform_attrs(cls, these, auto_attribs)
    ...
    def _patch_original_class(self):
        ...
        if self._delete_attribs:
            for name in self._attr_names:
                if name not in super_names and getattr(cls, name, None) is not None:
                    delattr(cls, name)
    ...
    def add_hash(self):
        self._cls_dict["__hash__"] = self._add_method_dunders(_make_hash(self._attrs))
    ...
    def add_cmp(self):
        ...
        cd["__eq__"], cd["__ne__"], cd["__lt__"], cd["__le__"], cd["__gt__"], cd[
            "__ge__"
...
def _make_hash(attrs):
    attrs = tuple(
        a for a in attrs if a.hash is True or (a.hash is None and a.cmp is True)
    )
    ...
    lines = ["def __hash__(self):", "    return hash((", "        %d," % (type_hash,)]
    for a in attrs:
...
def _attrs_to_init_script(attrs, frozen, slots, post_init, super_attr_map):
    ...
    """
    lines = []
    any_slot_ancestors = any(_is_slot_attr(a.name, super_attr_map) for a in attrs)
    if frozen is True:
        if slots is True:
            lines.append(
...
class _CountingAttr(object):
    ...
    *Internal* data structure of the attrs library.  Running into is most
    ...
    def __init__(
        self, default, validator, repr, cmp, hash, init, converter, metadata, type
    ):
